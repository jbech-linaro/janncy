...
const std::vector<int>& KernelAttributes::strides() const {
    return strides_;
}
const std::vector<int>& KernelAttributes::begin_pads() const {
    return begin_pads_;
}
const std::vector<int>& KernelAttributes::end_pads() const {
    return end_pads_;
}
...
std::vector<int> KernelAttributes::output_shape(
    ...
    PANIC_IF(input_shape.size() != kernel_shape_.size() + 1,
             "Incompatible input and kernel shapes",
             input_shape, kernel_shape_);
